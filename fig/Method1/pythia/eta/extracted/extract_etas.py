...
# if need to rebin the histrogram
def rebin(input_hist):
    hist = input_hist.Clone()
    a = np.array([])
    for j in range(1,hist.GetNbinsX()+2):
...
for k in range(7,13):   #for only dijet event, start from jet pT>500 GeV
    ...
    quark_hist_array_data = []
    gluon_hist_array_data = []
...
#for i in range(13):	#for gamma+jet combined with dijet event, start from jet pT>0 GeV
    ...
    ######################## for ratio plot
        ...
        quark_hist_array_data.append(quark_data)
        gluon_hist_array.append(gluon)
        gluon_hist_array_data.append(gluon_data)        
        quark_hist_array_ratio.append(ex_quark_ratio)
        gluon_hist_array_ratio.append(ex_gluon_ratio)
    
    fg_quark_mc_ratio = quark_hist_array[0].Clone()
    fg_quark_mc_ratio.Divide(quark_hist_array[1])
    fg_quark_data_ratio = quark_hist_array_data[0].Clone()
    fg_quark_data_ratio.Divide(quark_hist_array_data[1])
    fg_gluon_mc_ratio = gluon_hist_array[0].Clone()
    fg_gluon_mc_ratio.Divide(gluon_hist_array[1])
    fg_gluon_data_ratio = gluon_hist_array_data[0].Clone()
    fg_gluon_data_ratio.Divide(gluon_hist_array_data[1])
    ...
    for j in range(2):
            quark_hist_array_data[j].SetMarkerColor(3*j+1)
            quark_hist_array_data[j].SetLineColor(3*j+1)
            quark_hist_array_data[j].SetMarkerSize(0.5)
            quark_hist_array_data[j].SetLineStyle(1)
    ...
    for j in range(2):
            ...
            quark_hist_array_ratio[j].SetLineStyle(20)
    ...
    fg_quark_mc_ratio.SetLineStyle(20)  
    fg_quark_mc_ratio.SetLineColor(1)

    fg_quark_data_ratio.SetMarkerSize(0.5)
    fg_quark_data_ratio.SetLineStyle(20)  
    fg_quark_data_ratio.SetLineColor(1)
    
    top.cd()
    quark_hist_array[0].Draw("HIST e") #extracted mc
    quark_hist_array[1].Draw("HIST e same") 
    #quark_hist_array[2].Draw("HIST same") 
    quark_hist_array_data[0].SetMarkerStyle(20)
    quark_hist_array_data[1].SetMarkerStyle(20)

    quark_hist_array_data[0].Draw("E1P same") #extracted mc
    quark_hist_array_data[1].Draw("E1P same") 
    #quark_hist_array_data[2].Draw("p same") 
    ...
    leg.AddEntry(quark_hist_array_data[0],"extracted quark(data) \eta_{1}:"+eta_start_central_1+"<|\eta_{C}|<"+eta_end_central_1+","+eta_start_forward_1+"<|\eta_{C}|<"+eta_end_forward_1+"","p")
    leg.AddEntry(quark_hist_array_data[1],"extracted quark(data) \eta_{2}:"+eta_start_central_2+"<|\eta_{C}|<"+eta_end_central_2+","+eta_start_forward_2+"<|\eta_{C}|<"+eta_end_forward_2+"","p")
    leg.Draw()
    ...
    fg_quark_data_ratio.Draw("E1P e same")   
    line.Draw("same")
    bot.cd()
    ...
    for j in range(2):
            gluon_hist_array_data[j].SetMarkerColor(3*j+1)
            gluon_hist_array_data[j].SetLineColor(3*j+1)
            gluon_hist_array_data[j].SetMarkerSize(0.5)
            gluon_hist_array_data[j].SetLineStyle(1)
    ...
    for j in range(2):
            ...
            gluon_hist_array_ratio[j].SetLineStyle(20)
    ...
    fg_gluon_mc_ratio.SetLineStyle(20)  
    fg_gluon_mc_ratio.SetLineColor(1)

    fg_gluon_data_ratio.SetMarkerSize(0.5)
    fg_gluon_data_ratio.SetLineStyle(20)  
    fg_gluon_data_ratio.SetLineColor(1)
    
    top.cd()
    gluon_hist_array[0].Draw("HIST e") #extracted mc
    gluon_hist_array[1].Draw("HIST e same") 
    #gluon_hist_array[2].Draw("HIST same") 
    gluon_hist_array_data[0].SetMarkerStyle(20)
    gluon_hist_array_data[1].SetMarkerStyle(20)

    gluon_hist_array_data[0].Draw("E1P same") #extracted mc
    gluon_hist_array_data[1].Draw("E1P same") 
    #gluon_hist_array_data[2].Draw("p same") 
    ...
    leg.SetFillStyle(0)
    leg.SetNColumns(1)
    leg.AddEntry(gluon_hist_array[0],"extracted gluon(mc) \eta_{1}: "+eta_start_central_1+"<|\eta_{C}|<"+eta_end_central_1+","+eta_start_forward_1+"<|\eta_{F}|<"+eta_end_forward_1+"","l")
    leg.AddEntry(gluon_hist_array[1],"extracted gluon(mc) \eta_{2}: "+eta_start_central_2+"<|\eta_{C}|<"+eta_end_central_2+","+eta_start_forward_2+"<|\eta_{F}|<"+eta_end_forward_2+"","l")
    leg.AddEntry(gluon_hist_array_data[0],"extracted gluon(data) \eta_{1}:"+eta_start_central_1+"<|\eta_{C}|<"+eta_end_central_1+","+eta_start_forward_1+"<|\eta_{F}|<"+eta_end_forward_1+"","p")
    leg.AddEntry(gluon_hist_array_data[1],"extracted gluon(data) \eta_{2}:"+eta_start_central_2+"<|\eta_{C}|<"+eta_end_central_2+","+eta_start_forward_2+"<|\eta_{F}|<"+eta_end_forward_2+"","p")
    leg.Draw()
    myText(0.18,0.84,"#it{#bf{#scale[1.5]{#bf{ATLAS} Internal}}}")
    myText(0.18,0.80,"#bf{#scale[1.2]{#sqrt{s} = 13 TeV}}")
    myText(0.18,0.75,"#bf{#scale[1.2]{pT range: "+str(min)+" - "+str(max)+" GeV}}")
    ...
    fg_gluon_data_ratio.Draw("E1P e same")   
    line.Draw("same")
    bot.cd()
