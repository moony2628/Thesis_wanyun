...
\hspace{8mm}
这篇论文中所呈现的物理分析旨在寻找标准模型之外的双喷注不变质量分布中可能存在的新物理现象，使用由大型强子对撞机的ATLAS探测器记录的质子-质子碰撞数据, 其积分亮度为140 fb$^{-1}$，中心质心能量为13 TeV。该分析旨在寻找衰变为两个喷注的新共振态证据。为了增强检测可能主要衰变为一个或多个胶子的新共振的能力，采用了基于与喷注相关的带电轨迹的胶子标记方法。在相关分析中普遍使用一种名为夸克/胶子标记的方法，以区分起源于夸克和胶子的喷注的过程。

本研究开发了两种夸克/胶子喷注标记器，并研究了两种标记器的性能和校准：一种基于与喷注关联的带电径迹数目，而另一种则采用增强决策树来结合各种喷注次级结构可观测量，同时提供了从500 GeV到2 TeV的喷气横动量范围内，喷注横向动量提供标记效率的数据与蒙特卡洛模拟之间的差异，范围在0.92到1.02之间，包括大约20％的系统误差。该误差主要受到理论建模的影响。在数据上运用矩阵方法，从两个富含夸克/胶子地子样本中提取真实的夸克/胶子喷注比例，该样本由喷注的赝快度定义。

在分析中，通过参数化双喷注不变质量谱的统计框架估计量子色动力学本底，定义了三种不同的谱：未标记双喷注不变质量、一个胶子标记和两个胶子标记信号道。由于没有观察到与本底明显的偏差，因此对超越标准模型的物理的各种信号模型的三个类别在95％置信水平上设定了交叉截面的上限。通过在事件选择中加入胶子标记，考虑的新物理信号的搜索限制得到了谱上的改进。
